...
// Badness markers: present in wrong-but-plausible answers, absent in good ones.
const HEDGES: &[&str] = &[
...
const NEGATIVES: &[&str] = &[
    "wrong", "incorrect", "false", "scam", "crash", "crashing", "dead", "dump",
    "collapsed", "ponzi", "fake", "misleading",
];
...
/// Penalty (0..~0.6) for markers that signal a wrong/weak answer.
fn badness_penalty(ma: &str) -> f32 {
    let mut p = 0.0f32;
    if contains_any(ma, HEDGES) { p += 0.25; }
    if contains_any(ma, QUALIFIERS) { p += 0.15; }
    if contains_any(ma, NEGATIVES) { p += 0.20; }
    p
}

/// Score `ma` by SIMILARITY TO GROUND TRUTH `gt`, minus badness penalty.
/// Good answer (ma == gt) scores 1.0. Bad answer that matches gt numerically
/// but hedges/contradicts is penalized below good -> fixes ordering.
fn similarity_to_truth(gt: &str, ma: &str) -> f32 {
    ...
    if gn_n > 0 && an_n > 0 {
        ...
        return (base - badness_penalty(ma)).max(0.0).min(1.0);
    }
    ...
    (base - badness_penalty(ma)).max(0.0).min(1.0)
}
