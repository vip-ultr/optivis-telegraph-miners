...
/// Penalty for INTERNAL numeric contradiction: an answer listing two very
/// different prices/values for the same asset is self-conflicting -> wrong.
fn consistency_penalty(ma: &str) -> f32 {
    let mut nums = [0f64; MAX_NUMS];
    let n = extract_numbers(ma, &mut nums);
    if n < 2 { return 0.0; }
    // ratio of largest to smallest among extracted numbers
    let mut lo = f64::MAX;
    let mut hi = 0f64;
    for i in 0..n {
        let v = nums[i].abs();
    ...
    if lo == f64::MAX || hi == 0.0 { return 0.0; }
    let ratio = hi / lo;
    // two values differing by >5x in one short answer => likely conflicting figures
    if ratio > 5.0 { 0.30 } else if ratio > 2.0 { 0.15 } else { 0.0 }
}

/// Score `ma` by SIMILARITY TO GROUND TRUTH `gt`, minus internal-contradiction penalty.
/// Good answer (ma == gt) scores 1.0. Self-conflicting bad answers are penalized.
fn similarity_to_truth(gt: &str, ma: &str) -> f32 {
    ...
    if gn_n > 0 && an_n > 0 {
        ...
        let base = (num_sim * 0.8 + txt * 0.2).max(0.0).min(1.0);
        return (base - consistency_penalty(ma)).max(0.0).min(1.0);
    }
    ...
    if gn_n == 0 && an_n == 0 {
        let base = (token_score * 0.5 + txt * 0.5).max(0.0).min(1.0);
        return (base - consistency_penalty(ma)).max(0.0).min(1.0);
    }

    // One side has a number, the other doesn't.
    let base = (token_score * 0.4 + txt * 0.3).max(0.0).min(1.0);
    (base - consistency_penalty(ma)).max(0.0).min(1.0)
}
