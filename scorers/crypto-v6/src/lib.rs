...
/// Metrics that indicate a complete, high-quality answer (vs a sparse bad one).
const RICHNESS: &[&str] = &[
    "market cap", "marketcap", "mcap", "volume", "vol", "tvl", "total value locked",
    "supply", "circulating", "fdv", "fully diluted", "change", "24h", "24h%",
    "high", "low", "ath", "atl", "rank", "dominance", "liquidity",
];

/// Bonus (0..~0.15) for answers that cite multiple financial metrics -> complete.
fn richness_bonus(ma: &str) -> f32 {
    let mut hits = 0u32;
    for w in RICHNESS {
        if has_token(ma, w) { hits += 1; }
    }
    (hits as f32 * 0.05).min(0.15)
}

/// Score `ma` by SIMILARITY TO GROUND TRUTH `gt`, plus a richness bonus.
/// Good answer (ma == gt) scores 1.0. Detailed good answers beat sparse bad ones.
fn similarity_to_truth(gt: &str, ma: &str) -> f32 {
    ...
    if gn_n > 0 && an_n > 0 {
        ...
        return (base + richness_bonus(ma)).max(0.0).min(1.0);
    }
    ...
    (base + richness_bonus(ma)).max(0.0).min(1.0)
}
